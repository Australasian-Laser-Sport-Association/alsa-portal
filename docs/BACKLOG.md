...
- **P2 — Consolidate duplicate queries on the ZLTAC page.** Currently firing multiple overlapping `zltac_events` and `zltac_event_history` queries on page load. Consolidate or memoise with React Query / SWR.
- **P2 — Seed production with real ZLTAC event data.** Once registration is tested end-to-end, seed 2018-2025 historical events into `zltac_event_history` and create the 2026 event in `zltac_events`.
...
- **P2 — Write `ADR-0004: CMS removed in favour of static content.`** Captures today's decision to drop the three `cms_*` tables.
...
- **P3 — Import historical event history** (2018-2025) into `zltac_event_history` with champion teams, locations, MVPs, photos.
