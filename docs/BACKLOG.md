...
* **P2 — Fix `EventPage.jsx` admin-aware UI branch (dead code).** In `src/pages/EventPage.jsx` around line 395, the page does `supabase.from('profiles').select('role').eq('id', user.id).single()` — but the schema column is `roles` (plural), not `role`. The result is then read as `profileResult?.data?.roles` to compute `admin`, which is always `undefined` → `[]` → `admin === false`. Net effect: the admin-aware UI branch on the public event page is dead code today. Fix: change the select to `select('roles')` (drop the unused `role` singular extraction — already removed in `6399ade`). Verify the admin branch then renders as expected for committee users. Context: surfaced while doing the cleanup pass at commit `6399ade`.
