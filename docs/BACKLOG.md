...
* **P2 — Admin page load latency.** AdminRegistrations and AdminUsers take 2-4s to render on initial load. Likely contributors: (1) Vercel hobby-tier cold starts on `/api/admin/*` functions, (2) sequential queries inside the handlers — registrations → profiles → teams fetched one after the other rather than via `Promise.all`, (3) auth overhead — every request re-validates the JWT and re-fetches the requester's profile to check `is_committee()` before the real query runs. Audit `api/admin/registrations.js` and `api/admin/users.js`: parallelize independent queries, consider caching the `is_committee()` check per-request, and look for any N+1 query patterns. Cold starts are infra-level; the in-handler optimisations are the real win and likely halve the load time.
* **P2 — Strip doubles/triples slug from `side_events` on partner removal.** The `disband` / `clear-slot` (and doubles `delete`) actions in `api/player/triples.js` and `api/player/doubles.js` delete the pair/team record but leave the `'doubles'`/`'triples'` slug in each affected player's `zltac_registrations.side_events` array. Because `amount_owing` is computed from that array, players stay billed for a removed partner until they next re-confirm side events in PlayerHub. Fix: on those actions, remove the slug from each affected registration's `side_events`, then call `computeAndWriteAmountOwing` for each. Introduced with the Chunk 1/2 payment-tracking work.
