...
function HostingTile({ region, years, count, selected, onClick }) {
  return (
    ...
    >
      ...
      <p className="text-[#e5e5e5]/60 text-[11px] uppercase tracking-wider mb-2">{REGION_LABEL[region]}</p>
      <div className="flex flex-wrap gap-1">
        {years.map(y => (
          <span key={y} className="text-[10px] text-[#e5e5e5]/60 bg-line/40 px-1.5 py-0.5 rounded">
            {y}
          </span>
        ))}
      </div>
