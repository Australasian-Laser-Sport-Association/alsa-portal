...
-- Pass 1 security-audit remediations (DB layer).
-- Covers three confirmed findings:
--   #3 Critical  - anon can read bank details from zltac_events
--   #4 Critical  - players can tamper with admin-only registration columns
--   #1 Critical  - non-superadmin committee can rewrite profiles.roles via RLS
--
-- NOTE: The API half of #1 (/api/admin/users) and #2 (claim_placeholder_profile)
-- are handled separately in code.
...
--    let any committee member set roles (self-promote to superadmin) directly
