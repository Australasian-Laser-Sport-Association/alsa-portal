...
BEGIN
  IF v_contract_applied THEN
    IF to_regprocedure('public.can_read_team_members(uuid)') IS NOT NULL
       OR EXISTS (
         SELECT 1
           FROM pg_policies
       ...
       OR has_any_column_privilege(
         'authenticated', 'public.team_members', 'SELECT'
       ) THEN
      RAISE EXCEPTION 'retired team-member browser read path remains after 66000';
    END IF;
  ELSE
    IF to_regprocedure('public.can_read_team_members(uuid)') IS NULL THEN
      RAISE EXCEPTION 'can_read_team_members(uuid) is missing before 66000';
    END IF;
